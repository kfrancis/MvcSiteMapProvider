    ...
    : ISortable
{
    ...
    ///     If set to <c>true</c> the visibility of the current node will affect the descendant nodes.
    /// </param>
        ...
        bool drillDownToCurrent, bool startingNodeInChildLevel, bool? visibilityAffectsDescendants)
    {
        ...
        VisibilityAffectsDescendants = visibilityAffectsDescendants ?? false;
    ...
    public SiteMapNodeModelList Children
    {
        get
        {
            ...
            {
                _children = [];
                var reachedLevel = ReachedMaximalNodelevel(_maxDepth, _node, _drillDownToCurrent);
                if (reachedLevel && _node.HasChildNodes)
                {
                    // When we are allowed to include children due to drill-down, treat depth 0 as allowing one level.
                    var depthForChildren = _maxDepth > 0 ? _maxDepth : 1;
                    var nextDepth = _maxDepth > 0 ? _maxDepth - 1 : 0; // Prevent negative depths.
                    ...
                    if (VisibilityAffectsDescendants)
                    {
                        ...
                        {
                            if (child.IsAccessibleToUser() && child.IsVisible(SourceMetadata) && depthForChildren > 0)
                            {
                                _children.Add(new SiteMapNodeModel(child, SourceMetadata, nextDepth,
                    ...
                    else
                    {
                        foreach (var child in sortedNodes)
                        {
                            ...
                            if (child.IsVisible(SourceMetadata) && depthForChildren > 0)
                            {
                                _children.Add(new SiteMapNodeModel(child, SourceMetadata, nextDepth,
                                    _drillDownToCurrent, false, VisibilityAffectsDescendants));
                            }
                            else if (depthForChildren > 1) // allow visibility substitution only if more than one level remaining
                            {
                                var nearestVisibleDescendants = new List<SiteMapNodeModel>();
                                FindNearestVisibleDescendants(child, nextDepth, ref nearestVisibleDescendants);
