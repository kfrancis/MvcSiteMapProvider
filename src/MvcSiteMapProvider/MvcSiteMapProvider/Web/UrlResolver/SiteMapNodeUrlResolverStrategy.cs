...
namespace MvcSiteMapProvider.Web.UrlResolver
{
        ...
        : ISiteMapNodeUrlResolverStrategy
    {
        ...
        {
            var provider = Array.Find(siteMapUrlResolvers, x => x.AppliesTo(providerName));
            if (provider == null)
            {
                ...
                provider = Array.Find(siteMapUrlResolvers, x => x.GetType().Equals(typeof(SiteMapNodeUrlResolver)));
            }
            return provider;
        }
