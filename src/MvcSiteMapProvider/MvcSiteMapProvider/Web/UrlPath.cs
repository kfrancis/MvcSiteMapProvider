...
namespace MvcSiteMapProvider.Web
{
        ...
        : IUrlPath
    {
        ...
        protected virtual bool IsVisualStudioDevelopmentServer =>
            string.IsNullOrEmpty(HttpContext.Request.ServerVariables["SERVER_SOFTWARE"]);

        public string AppDomainAppVirtualPath => HttpContext.Request.ApplicationPath;
        ...
        public string CombineUrl(params string[]? uriParts)
        {
            ...
            {
                uri = $"{uri.TrimEnd(trims)}/{uriParts[i].TrimStart(trims)}";
            }
        ...
        public bool IsAbsoluteUrl(string url)
        {
            ...
            return url.IndexOf(Uri.SchemeDelimiter, StringComparison.Ordinal) > 0 &&
                   ...
                   Regex.IsMatch(url, @"^\w+://", RegexOptions.Compiled);
        }

        /// <summary>
        ///     Determines if a URL is not part of the current application or website.
        /// </summary>
        /// <param name="url">Any Url including those starting with "/", "~", or protocol.</param>
        /// <param name="httpContext">The HTTP context representing the request.</param>
        /// <returns>
        ///     <b>true</b> if the URL is not part of the virtual application or is on a different host name; otherwise
        ///     <b>false</b>.
        /// </returns>
        public bool IsExternalUrl(string url, HttpContextBase httpContext)
        {
            ...
            var isDifferentVirtualApplication = httpContext.Request.ApplicationPath != null &&
                                                !uri.AbsolutePath.StartsWith(httpContext.Request.ApplicationPath,
        ...
        /// <param name="url">Any Url including those starting with "/", "~", or protocol.</param>
        /// <param name="protocol">
        ///     The protocol such as http, https, or ftp. Defaults to http
        ///     protocol if null or empty string. To use the protocol of the current request, use *.
        /// </param>
        /// <returns>The resolved URL.</returns>
        ...
        /// <param name="url">Any Url including those starting with "/", "~", or protocol.</param>
        /// <param name="protocol">
        ///     The protocol such as http, https, or ftp. Defaults to protocol of the
        ///     request to prevent errors when resolving content under https.
        /// </param>
        /// <returns>The resolved URL.</returns>
        ...
        /// <remarks>
        ///     Source:
        ///     http://stackoverflow.com/questions/7795910/how-do-i-get-url-action-to-use-the-right-port-number#11888846
        /// </remarks>
        public Uri GetPublicFacingUrl(HttpContextBase httpContext)
        {
            ...
            // appear on the public-facing side of the hosting website.
        ...
        public string ResolveServerUrl(string serverUrl, bool forceHttps)
        {
            ...
            {
                var scheme = httpContext.Request.Headers["HTTP_X_FORWARDED_PROTO"] ?? httpContext.Request.Url?.Scheme;
        ...
        ///     This method returns a fully qualified absolute server Url which includes
        ///     the protocol, server, port in addition to the server relative Url.
        ///     It works like Page.ResolveUrl, but adds these to the beginning.
        ...
        private void CheckValidVirtualPath(string path)
        {
            ...
            {
                throw new HttpException(string.Format(Messages.PhysicalPathNotAllowed, path));
            }
            ...
            if (HasScheme(path))
            {
                throw new HttpException(string.Format(Messages.InvalidVirtualPath, path));
            }
        }
        ...
        private string ReduceVirtualPath(string path)
        {
            ...
            while (true)
            {
                ...
                if ((startIndex == 0 || path[startIndex - 1] == '/') &&
                    (startIndex + 1 == length ||
                     path[startIndex + 1] == '/' ||
                     path[startIndex + 1] == '.' && (startIndex + 2 == length || path[startIndex + 2] == '/')))
                {
                    break;
                }
            ...
            do
            {
                ...
                if (startIndex - num3 <= 3 &&
                    (startIndex < 1 || path[startIndex - 1] == '.') &&
                    (num3 + 1 >= length || path[num3 + 1] == '.'))
                {
                    if (startIndex - num3 != 3)
                    {
                        continue;
                    ...
                    {
                        case 0:
                            throw new HttpException(Messages.CannotExitUpTopDirectory);

                        case 1 when IsAppRelativePath(path):
        ...
        /// <param name="url">Any Url including those starting with "/", "~", or protocol.</param>
        /// <param name="protocol">
        ///     The protocol such as http, https, or ftp.
        ///     To use the protocol of the current request, use *.
        /// </param>
        /// <param name="hostName">The host name such as www.somewhere.com.</param>
        ...
        /// <param name="httpContext">The HTTP context representing the context of the request.</param>
        /// <returns>The resolved URL.</returns>
        protected virtual string GenerateUrl(string url, string protocol, string hostName, bool defaultToHttp,
        ...
        protected virtual string GetPortString(string protocol, string hostName, Uri requestUrl)
        {
            ...
            else
            {
                ...
                {
                    // Match the protocol
                    var protocolBindings = bindings
                        .Where(x => string.Equals(x.Protocol, protocol, StringComparison.OrdinalIgnoreCase)).ToList();
            ...
            return IsDefaultPort(port, protocol)
                ? string.Empty
                : ":" + Convert.ToString(port, CultureInfo.InvariantCulture);
        }
