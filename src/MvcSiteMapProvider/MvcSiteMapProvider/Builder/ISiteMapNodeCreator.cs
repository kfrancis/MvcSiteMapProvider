...
public interface ISiteMapNodeCreator
{
    ...
    /// <param name="implicitResourceKey">The implicit resource key for localization of the node's properties.</param>
    /// <returns>An <see cref="T:MvcSiteMapProvider.Builder.ISiteMapNodeToParentRelation" /> instance.</returns>
    ISiteMapNodeToParentRelation CreateDynamicSiteMapNode(string key, string parentKey, string sourceName,
        string implicitResourceKey);


    /// <summary>
    ///     Generates a key based on the properties that are passed. The combination of values should be unique within the
    ///     sitemap.
    /// </summary>
    /// <param name="parentKey">The key of the intended parent node.</param>
    ...
    string GenerateSiteMapNodeKey(string parentKey, string key, string url, string title, string area,
