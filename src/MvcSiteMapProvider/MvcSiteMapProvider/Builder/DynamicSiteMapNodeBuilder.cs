...
/// <summary>
///     Creates nodes with a map to their parent node dynamically based on an implementation of
///     <see cref="T:MvcSiteMapProvider.IDynamicNodeProvider" />.
/// </summary>
    ...
    : IDynamicSiteMapNodeBuilder
{
    ...
    private readonly ISiteMapNodeCreator _siteMapNodeCreator;
    ...
    {
        _siteMapNodeCreator = siteMapNodeCreator ?? throw new ArgumentNullException(nameof(siteMapNodeCreator));
        _cultureContext = cultureContext ?? throw new ArgumentNullException(nameof(cultureContext));
    ...
    public virtual IEnumerable<ISiteMapNodeToParentRelation> BuildDynamicNodes(ISiteMapNode node,
        string? defaultParentKey)
    {
        ...
        using (_cultureContextFactory.Create(_cultureContext.OriginalCulture, _cultureContext.OriginalUICulture))
        {
        ...
        foreach (var dynamicNode in dynamicNodes)
        {
            ...
            var parentKey = !string.IsNullOrEmpty(dynamicNode.ParentKey)
                ? dynamicNode.ParentKey
                : defaultParentKey ?? string.Empty;
            var key = dynamicNode.Key;

            if (string.IsNullOrEmpty(key))
            {
                key = _siteMapNodeCreator.GenerateSiteMapNodeKey(
                    parentKey,
            ...
            // Create a new node
            var nodeParentMap =
                _siteMapNodeCreator.CreateDynamicSiteMapNode(key, parentKey, node.DynamicNodeProvider,
                    node.ResourceKey);
